...
def identify_preftcher_and_benchmark(filepath, file_map):
    ...
    # Try to find prefetcher key in filename segments
    for key, name in file_map.items():
        # Check common separators and ensure it's surrounded by separators or start/end
        if re.search(r'[-_.]' + re.escape(key) + r'[-_.]', fname):
            # Assume benchmark name is the part before .champsimtrace.xz
            # Refined regex to handle different separators around benchmark name
            match = re.search(r'(?:---|\.\.\.|[-_])(.*?)\.champsimtrace\.xz', fname)
            if match:
                benchmark = match.group(1).strip('-_') # Clean up separators
                # Further clean common benchmark naming patterns if needed
                benchmark = re.sub(r'^\d+core[-_]+', '', benchmark) # Remove core count prefix if present
                return name, benchmark
            else:
                 # Fallback: try to find common benchmark patterns if specific separator fails
                common_bench_match = re.search(r'(\d{3}\..*?-\d+B)', fname)
                if common_bench_match:
                    benchmark = common_bench_match.group(1)
                    return name, benchmark

    print(f"Warning: Could not identify prefetcher/benchmark for {fname}")
    return None, None
...
def parse_output_files(input_dir):
    ...
    if not output_files:
        output_files = list(input_path.glob('*.xz')) # Check for .xz as well
        if not output_files:
            # Last resort: glob everything, then filter for files
            all_items = list(input_path.glob('*'))
            output_files = [f for f in all_items if f.is_file()]
            if not output_files:
                print(f"Error: No files found in directory '{input_dir}'. Searched for *.txt, *.xz, and *")
                sys.exit(1)
            else:
                print("Warning: Found files using '*', ensure these are ChampSim outputs.")
        else:
            print("Info: Found files ending in .xz, assuming these are ChampSim outputs.")
    else:
        print("Info: Found files ending in .txt.")
    ...
    if baseline_df.empty:
        print("Error: 'No-Prefetch' baseline data not found. Cannot calculate Speedup or Coverage.")
        # Still return the parsed data, but without derived metrics dependent on baseline
    else:
...
if __name__ == "__main__":
    ...
    if results_df is not None:
        ...
        try:
            ...
            print("\nDataFrame Info:")
            results_df.info() # This prints to stdout directly
